const ScoreKey = "tetris-score-key"

function getBestScore() {
   return localStorage.getItem(ScoreKey) || '0';
}

function updateBestScore() {
    score = parseInt(document.getElementById("score").innerText)
    bestScore = parseInt(getBestScore())
    if(score > bestScore){
...
class Board {
    ...
    initTileContainer(){
        ...
        for(let i = 0; i < this.tileCount; i++){
            ...
            emptyDiv.className = "bg-empty"

            const innerTile = document.createElement('div')
            innerTile.className = "bg-inner-tile"

            const innerInnerTile = document.createElement('div')
            innerInnerTile.className = "bg-inner-inner-tile"

            innerTile.appendChild(innerInnerTile)
            emptyDiv.appendChild(innerTile)
            tileContainer.appendChild(emptyDiv)
        }
    }
